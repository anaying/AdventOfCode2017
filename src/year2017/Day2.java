...
public class Day2 {
    ...
    private static int part1(String input) {
        ...
        while (scLine.hasNextLine()) {
            ...
            Scanner scTab = new Scanner(line).useDelimiter("\t");
            ArrayList<Integer> list = new ArrayList<>();
            while (scTab.hasNext()) {
                list.add(scTab.nextInt());
            }
            sol += Collections.max(list) - Collections.min(list);
        }
        return sol;
    }
            ...
            for (int i = 0; i < list.size(); i++) {
                for (int j = i + 1; j < list.size(); j++) {
                    if (list.get(i) % list.get(j) == 0) {
                        sol += list.get(i) / list.get(j);
                        break;
                    } else if (list.get(j) % list.get(i) == 0) {
                        sol += list.get(j) / list.get(i);
                        break;
                    }
                }
            }
