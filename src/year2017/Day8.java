...
//            c inc -20 if c == 10""";
    ...
    public static void main(String[] args){
        ...
        while (sc.hasNextLine()) {
            String[] line = sc.nextLine().split(" if ");
            String[][] command = {line[0].split(" "),line[1].split(" ")};
            if (!map.containsKey(command[0][0])) {
                map.put(command[0][0], 0);
            }
            if(!map.containsKey(command[1][0])) {
                map.put(command[1][0], 0);
            }
            if (condition(command[1]))
                instruction(command[0]);
            if(map.get(command[0][0]) > max)
    ...
    private static void instruction(String[] instruction) {
        if (instruction[1].equals("inc"))
            map.put(instruction[0], map.get(instruction[0]) + Integer.parseInt(instruction[2]));
        else
            map.put(instruction[0], map.get(instruction[0]) - Integer.parseInt(instruction[2]));
    }
